...
<%--                                <div class="input-group">购买数量--%>
<%--            <i class = "minus iconfont icon-jianhao"></i>--%>
<%--            <input type="number" min = "1" step="1" value="1" style="width: 50px" id="${entity.id}">--%>
<%--            <i class = "plus iconfont icon-jiahao1"></i>--%>
<%--                                    &nbsp;&nbsp;--%>
<%--                                <a class="btn btn-danger" type="submit" onclick="addCart(${entity.id})" >加入购物车</a>--%>
<%--                                </div>--%>
...
<script type="text/javascript">
    ...
    function goods_infolist() {
        window.location.href = "<%=path %>/goods/selectByPage?currentPage=" + currentPage + "&pageSize=" + pageSize +
            "&goodsname=" + goodsname + "&type=" + type ;
    }
    ...
    function addCart(goods_id) {
        var num = $('#' + goods_id).val();
        // alert(goods_id);
        // alert(num);
        if (confirm("确定加入购物车吗？") && "${current_user.type}" === "用户") {
            window.location.href = "<%=path %>/goods/addCart?user_id=" + "${current_user.id}" + "&goods_id=" + goods_id +
                "&num=" + num;
        }
    }
